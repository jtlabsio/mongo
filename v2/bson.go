...
func iterateProperties[T ~map[string]any](parentPrefix string, properties T, ft *map[string]string) {
...
func parseBSONSchema(schema bson.M) map[string]string {
	...
	if js, ok := schema["$jsonSchema"]; ok {
		schema, ok = js.(bson.M)
		if !ok {
			// if $jsonSchema is not a bson.M, return empty
	...
	if properties, ok := schema["properties"]; ok {
		if properties, ok := properties.(bson.M); ok {
...
func detectStringComparisonOperator(field string, values []string, bsonType string) bson.M {
	...
	// contains...
	if c {
		return bson.M{field: bson.Regex{
			Pattern: value,
			Options: "i",
		}}
	}
