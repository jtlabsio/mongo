...
func (qb QueryBuilder) FindOptions(qo queryoptions.Options) (*options.FindOptionsBuilder, error) {
	opts := options.Find()
...
func (qb QueryBuilder) setPaginationOptions(pagination map[string]int, opts *options.FindOptionsBuilder) {
	// check for limit
...
func (qb QueryBuilder) setProjectionOptions(fields []string, opts *options.FindOptionsBuilder) error {
...
func (qb QueryBuilder) setSortOptions(fields []string, opts *options.FindOptionsBuilder) error {
	if len(fields) > 0 {
		sort := map[string]int{}
		for _, field := range fields {
			val := 1
